"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AnimalTypeModule = void 0;
const common_1 = require("@nestjs/common");
const animal_type_service_1 = require("./animal_type.service");
const animal_type_controller_1 = require("./animal_type.controller");
const mongoose_1 = require("@nestjs/mongoose");
const animal_type_schema_1 = require("./schemas/animal_type.schema");
let AnimalTypeModule = exports.AnimalTypeModule = class AnimalTypeModule {
};
exports.AnimalTypeModule = AnimalTypeModule = __decorate([
    (0, common_1.Module)({
        imports: [
            mongoose_1.MongooseModule.forFeature([
                { name: animal_type_schema_1.AnimalType.name, schema: animal_type_schema_1.AnimalTypeSchema },
            ]),
        ],
        controllers: [animal_type_controller_1.AnimalTypeController],
        providers: [animal_type_service_1.AnimalTypeService],
    })
], AnimalTypeModule);
//# sourceMappingURL=animal_type.module.js.map